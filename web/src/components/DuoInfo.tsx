...
export function DuoInfo({
  ...
  useEffect(() => {
    fetch(`https://nlwesports.onrender.com/games/${id}/ads`)
  ...
  useEffect(() => {
    fetch(`https://nlwesports.onrender.com/ads/${id}/discord`)
      .then((response) => response.json())
  ...
  return (
    <div className=" keen-slider__slide p-2 rounded text-center bg-slate-900">
      ...
      <div className="text-md text-zinc-400">
        ...
        <span
          className={
            useVoiceChannel === false ? "text-red-600" : "text-emerald-500"
          }
        >
          {" "}
          {useVoiceChannel ? "Yes" : "No"}
        </span>
      </div>
    </div>
  );
